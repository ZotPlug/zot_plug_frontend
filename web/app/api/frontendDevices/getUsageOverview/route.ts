...
export async function POST(req: NextRequest) {
    const body: { userId?: string; deviceId?: string } = await req.json()
    const { userId, deviceId } = body

    if (!userId) return NextResponse.json({ ok: false, message: "Missing userId" })

    try {
        const res = await api.fetchJSON({ endpoint: `/api/devices/getUsageOverview?userId=${userId}${deviceId ? `&deviceId=${deviceId}` : ""}`, method: "GET" })
